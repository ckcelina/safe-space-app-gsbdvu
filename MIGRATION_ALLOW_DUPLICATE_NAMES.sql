...
-- This allows users to add multiple people with the same name (e.g., multiple "Dad" entries)
-- Run this in your Supabase SQL Editor

-- Drop the unique constraint if it exists
-- Note: The constraint name may vary, so we'll try common patterns
DO $$ 
BEGIN
...
-- This ensures each person has a unique identifier
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint 
...
-- Ensure RLS policies are still in place and properly scoped
-- These policies ensure multi-user data safety
DO $$
BEGIN
...
DO $$
BEGIN
    RAISE NOTICE '✅ Migration completed: Duplicate names are now allowed in persons table';
    RAISE NOTICE '✅ Each person is uniquely identified by their UUID (id column)';
    RAISE NOTICE '✅ RLS policies ensure data isolation between users';
END $$;
</parameter>
</invoke>

Now let me update the home screen to remove duplicate checking and allow multiple people with the same name:
...
export default function HomeScreen() {
    ...
    try {
      ...
      const personsWithMessages = await Promise.all(
        personsData.map(async (person) => {
          try {
            // CRITICAL: Always filter by BOTH user_id AND person_id
  ...
  const handleDeletePerson = useCallback(async (personId: string) => {
    ...
    try {
      ...
      // This ensures we never delete another user's data
      ...
      // CRITICAL: Delete person with BOTH id AND user_id filter
      // This ensures we never delete another user's person
   ...
   * - All people appear in a single unified list under "People"
   * - Ordered by most recently added (created_at DESC)
   * - No grouping by Family/Friends/etc.
   */
  const filteredPersons = useMemo(() => {
    ...
    // Deduplicate by person.id (UUID) - this is the ONLY unique identifier
    const seenPersonIds = new Set<string>();
    const dedupedPersons: PersonWithLastMessage[] = [];
    
    filtered.forEach((person) => {
      // Use person.id as the unique key (UUID)
      if (!person.id) {
        console.warn('[Home] Person missing ID:', person.name);
        return;
      }
      
      // Skip if we've already processed this person ID
      if (seenPersonIds.has(person.id)) {
        console.warn('[Home] Skipping duplicate person ID:', person.id, person.name);
        return;
      }

      // Mark this person ID as seen
   ...
   * - No error code 23505 handling needed (constraint removed)
   */
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      // Users can add multiple people with the same name (e.g., multiple "Dad" entries)
      
      // Insert new person directly
      const personData = {
        user_id: userId,
        name: trimmedName,
      ...
      if (error) {
        ...
        // Generic error handling (no special case for duplicate names)
        if (isMountedRef.current) {
  ...
  /**
   * FIXED: Handle subject/topic creation WITHOUT duplicate checking
   * - Duplicate topic names are now ALLOWED
   * - Each topic is uniquely identified by UUID (person.id)
   */
  const handleSaveSubject = useCallback(async () => {
    ...
    try {
      ...
      // Insert new topic person directly
